...
{
  auto f_GenCooldownSort = [](const auto& a, const auto& b) -> bool
  {
    return a.skill->getCooldown() < b.skill->getCooldown();
  };

  auto f_GenQDCostSort = [](const auto& a, const auto& b) -> bool
  {
    return a.skill->getCost() < b.skill->getCost();
  };

  auto f_GenSkillIDSort = [](const auto& a, const auto& b) -> bool
  {
    return a.skill->getID() < b.skill->getID();
  };

  auto f_GenSkillNameSort = [](const auto& a, const auto& b) -> bool
  {
    return a.skill->getName() < b.skill->getName();
  };

  auto f_GenSkillPrimSort = [](const auto& a, const auto& b) -> bool
  {
    return static_cast<uint32_t>(a.skill->getPrimary()) <
           static_cast<uint32_t>(b.skill->getPrimary());
  };

  auto f_GenSkillSecdSort = [](const auto& a, const auto& b) -> bool
  {
    return static_cast<uint32_t>(a.skill->getSecondary()) <
           static_cast<uint32_t>(b.skill->getSecondary());
  };

  auto f_GenSkillPointSort = [](const auto& a, const auto& b) -> bool
  {
    return a.skill->getValue() < b.skill->getValue();
  };

  auto f_GenSkillLevelSort = [](const auto& a, const auto& b) -> bool
  {
    return a.level_available < b.level_available;
  };

  auto f_GenEnabledSort = [](const auto& a, const auto& b) -> bool
  {
  ...
  if(sort_type == SkillSorts::COOLDOWN)
    std::sort(begin(skill_elements), end(skill_elements), f_GenCooldownSort);
  else if(sort_type == SkillSorts::COST)
    std::sort(begin(skill_elements), end(skill_elements), f_GenQDCostSort);
  else if(sort_type == SkillSorts::ID)
    std::sort(begin(skill_elements), end(skill_elements), f_GenSkillIDSort);
  else if(sort_type == SkillSorts::NAME)
    std::sort(begin(skill_elements), end(skill_elements), f_GenSkillNameSort);
  /* Sort the SSEs by their Skill's primary elemental attribute
    ...
    std::sort(begin(skill_elements), end(skill_elements), f_GenSkillPrimSort);
  /* Sort the SSEs by their Skill's secondary elemental attribute
       Ascending Order: NONE, PHYS, TH, PO, PR, CY, DI, VO */
  else if(sort_type == SkillSorts::SECONDARY)
    std::sort(begin(skill_elements), end(skill_elements), f_GenSkillSecdSort);
  else if(sort_type == SkillSorts::POINT_VALUE)
    std::sort(begin(skill_elements), end(skill_elements), f_GenSkillPointSort);
  else if(sort_type == SkillSorts::LEVEL_REQ)
    std::sort(begin(skill_elements), end(skill_elements), f_GenSkillLevelSort);
  else if(sort_type == SkillSorts::ENABLED)
    std::sort(begin(skill_elements), end(skill_elements), f_GenEnabledSort);
